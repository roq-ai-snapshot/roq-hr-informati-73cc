...
function PayrollViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PayrollInterface>(
    ...
    () =>
      getPayrollById(id, {
        relations: ['employee_payroll_employee_idToemployee'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Employee Payroll Employee Id Toemployee"
                    text={
                      <Link as={NextLink} href={`/employees/view/${data?.employee_payroll_employee_idToemployee?.id}`}>
                        {data?.employee_payroll_employee_idToemployee?.id}
                      </Link>
                    }
                  />
                )}
