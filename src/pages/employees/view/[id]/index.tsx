...
function EmployeeViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<EmployeeInterface>(
    ...
    () =>
      getEmployeeById(id, {
        relations: ['user', 'company', 'payroll_employee_payroll_idTopayroll'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('payroll', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Payroll Employee Payroll Id Topayroll"
                    text={
                      <Link as={NextLink} href={`/payrolls/view/${data?.payroll_employee_payroll_idTopayroll?.id}`}>
                        {data?.payroll_employee_payroll_idTopayroll?.salary}
                      </Link>
                    }
                  />
                )}
